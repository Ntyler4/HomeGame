...
function Avatar({ name, url, size=40 }:{ name:string; url?:string|null; size?:number }) {
  ...
  useEffect(()=>{
    ...
    if(!db)return;
    db.from("profiles").select("avatar_url").ilike("display_name",name).limit(1).then(({data})=>{
      const u=data?.[0]?.avatar_url||null;
      avatarCache[key]=u;
      setSrc(u);
    });
  },[name,url]);
    ...
    <div style={{ minHeight:"100vh",background:"#0A0A0A",display:"flex",alignItems:"center",justifyContent:"center",padding:24,position:"relative",overflow:"hidden" }}>
      ...
      <div style={{ width:"100%",maxWidth:400,position:"relative",zIndex:1 }}>
        ...
        <Card style={{ padding:0,overflow:"hidden" }}>
          {tab!=='reset'
            ?<div style={{ display:"flex",borderBottom:"1px solid rgba(201,168,76,0.15)" }}>{(['login','signup'] as const).map(t=><button key={t} onClick={()=>{setTab(t);setErr("");setMsg("");}} style={{ flex:1,padding:"14px 0",background:"none",border:"none",color:tab===t?"#C9A84C":"#555",fontFamily:"'Space Mono',monospace",fontSize:12,letterSpacing:1.5,textTransform:"uppercase",cursor:"pointer",borderBottom:tab===t?"2px solid #C9A84C":"2px solid transparent" }}>{t==='login'?'Sign In':'Create Account'}</button>)}</div>
            :<div style={{ borderBottom:"1px solid rgba(201,168,76,0.15)",padding:"12px 20px",display:"flex",alignItems:"center",gap:10 }}><button onClick={()=>{setTab('login');setErr("");setMsg("");}} style={{ background:"none",border:"none",color:"#555",fontSize:18,cursor:"pointer" }}>←</button><span style={{ color:"#888",fontFamily:"'Space Mono',monospace",fontSize:12 }}>RESET PASSWORD</span></div>}
      ...
      {!loading&&ordered.map((lg:any)=>{
        ...
        return<div key={lg.id} style={{ background:"rgba(255,255,255,0.03)",border:`1px solid ${isPinned?"rgba(201,168,76,0.3)":"rgba(201,168,76,0.12)"}`,borderRadius:14,padding:"13px 14px",marginBottom:9,display:"flex",alignItems:"center",gap:11 }}>
          ...
          <div onClick={()=>onSelectLeague(lg)} style={{ flex:1,minWidth:0,cursor:"pointer" }}>
            <div style={{ color:"#fff",fontFamily:"'Playfair Display',serif",fontSize:16,display:"flex",alignItems:"center",gap:5 }}>{lg.name} {isComm&&<span style={{ fontSize:12 }}>👑</span>}</div>
...
// ─── SESSION DETAIL (view + commissioner edit) ──────────
function SessionDetailView({ session, league, players, isCommissioner, onBack, onSaved, showToast }:any){
  ...
  useEffect(()=>{
    if(!db)return;
    db.from("session_entries").select("*, players(name)").eq("session_id",session.id).then(({data})=>{
      const e=data||[];setEntries(e);
      const profits:Record<string,string>={};e.forEach((en:any)=>{profits[en.id]=String(en.profit||0);});setEditedProfits(profits);
      setLoading(false);
    });
  },[session.id]);
  ...
  const handleSave=async()=>{
    ...
    await db.from("sessions").update({winner_name:winnerName,chicken_dinner_name:chickenDinner||null,buy_in_amount:Number(buyInAmount)||null}).eq("id",session.id);
    for(const e of entries){
      const newProfit=Number(editedProfits[e.id]||0);
      const diff=newProfit-(e.profit||0);
      await db.from("session_entries").update({profit:newProfit}).eq("id",e.id);
      if(diff!==0)await db.from("players").update({total_profit:db.rpc?undefined:undefined}).eq("id",e.player_id);
    }
  ...
  return(
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <Card style={{ marginBottom:12 }}>
        <div style={{ display:"flex",gap:12,marginBottom:editing?12:0 }}>
          <div style={{ flex:1,textAlign:"center" }}><div style={{ color:"#555",fontSize:9,fontFamily:"'Space Mono',monospace",letterSpacing:2 }}>POT</div><div style={{ color:"#C9A84C",fontSize:26,fontFamily:"'Space Mono',monospace",fontWeight:700,marginTop:3 }}>${session.pot}</div></div>
          <div style={{ flex:1,textAlign:"center" }}><div style={{ color:"#555",fontSize:9,fontFamily:"'Space Mono',monospace",letterSpacing:2 }}>BUY-IN</div><div style={{ color:"#888",fontSize:26,fontFamily:"'Space Mono',monospace",fontWeight:700,marginTop:3 }}>${session.buy_in_amount||league.buy_in}</div></div>
        </div>
        {editing&&<div style={{ display:"flex",gap:10 }}><div style={{ flex:1 }}><label style={{ color:"#888",fontSize:10,fontFamily:"'Space Mono',monospace",marginBottom:5,display:"block" }}>BUY-IN AMOUNT</label><input type="number" value={buyInAmount} onChange={e=>setBuyInAmount(e.target.value)} style={inp}/></div></div>}
      </Card>
      <Card style={{ marginBottom:12 }}>
      ...
      <Card style={{ marginBottom:12 }}>
        ...
        <div style={{ display:"flex",gap:10 }}>
          <div style={{ flex:1 }}>
            <div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace",marginBottom:6 }}>🏆 WINNER</div>
...
function TransferCommView({ league, players, profile, onBack, onTransferred }:any){
  ...
  const handleTransfer=async()=>{if(!db||!selected)return;setLoading(true);const{data:np}=await db.from("profiles").select("id").ilike("display_name",selected).limit(1);await db.from("leagues").update({commissioner_name:selected,commissioner_id:np?.[0]?.id||null}).eq("id",league.id);onTransferred();};
  return(
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      <BackButton onBack={onBack}/><SectionTitle text="Transfer Commissioner"/>
      <div style={{ color:"#888",fontSize:12,lineHeight:1.7,marginBottom:16 }}>Choose who takes over as commissioner before you leave.</div>
...
function LiveSessionView({ session, liveEntries, players, profile, isCommissioner, league, onBack, onSubmitEntry, onEndSession }:any){
  const [elapsed,setElapsed]=useState(0);const [myBuyIn,setMyBuyIn]=useState(league.buy_in||20);const [myRebuys,setMyRebuys]=useState(0);const [myCashOut,setMyCashOut]=useState(0);
  ...
  const handleEnd=async()=>{setSaving(true);const entries=liveEntries.map((e:any)=>({player_id:e.player_id,player_name:e.player_name,buy_in:e.buy_in||0,rebuys:e.rebuys||0,cash_out:isCommissioner?(cashOuts[e.player_id]||e.cash_out||0):(e.cash_out||0),profit:(isCommissioner?(cashOuts[e.player_id]||e.cash_out||0):(e.cash_out||0))-((e.buy_in||0)+(e.rebuys||0))}));await onEndSession({entries,elapsed,chickenDinner});setSaving(false);};
  ...
  return(
    <div style={{ padding:"16px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      {/* EVERY player submits buy-in, rebuys, AND cash-out */}
      {myPlayer&&<Card style={{ marginBottom:12,border:"1px solid rgba(201,168,76,0.3)",background:"rgba(201,168,76,0.04)" }}>
        <div style={{ color:"#C9A84C",fontSize:10,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:11 }}>YOUR STATS</div>
        ...
        <div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace",marginBottom:9 }}>
          Profit: <span style={{ color:myCashOut-(myBuyIn+myRebuys)>=0?"#4CAF8C":"#E05555",fontWeight:700 }}>{myCashOut-(myBuyIn+myRebuys)>=0?"+":""}${myCashOut-(myBuyIn+myRebuys)}</span>
        </div>
        <button onClick={handleSubmit} disabled={saving} style={{ width:"100%",padding:"9px 0",background:"rgba(201,168,76,0.15)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:9,color:"#C9A84C",fontFamily:"'Space Mono',monospace",fontSize:11,letterSpacing:1.5,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:7 }}>{saving?<Spinner size={13}/>:"SUBMIT MY STATS ✓"}</button>
      </Card>}
      <Card style={{ marginBottom:12 }}>
        <div style={{ color:"#888",fontSize:10,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:11 }}>IN SESSION ({liveEntries.length})</div>
...
function NewSessionView({ league, players, onStart, onBack }:any){
  ...
  return(
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <Card style={{ marginBottom:12 }}>
        <div style={{ color:"#888",fontSize:10,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:9 }}>BUY-IN FOR TONIGHT</div>
        ...
        <div style={{ display:"flex",gap:5,flexWrap:"wrap" }}>{[10,20,25,50,100].map(amt=><button key={amt} onClick={()=>setSessionBuyIn(amt)} style={{ padding:"4px 10px",borderRadius:20,background:sessionBuyIn===amt?"rgba(201,168,76,0.2)":"rgba(255,255,255,0.04)",border:`1px solid ${sessionBuyIn===amt?"rgba(201,168,76,0.4)":"rgba(255,255,255,0.08)"}`,color:sessionBuyIn===amt?"#C9A84C":"#555",fontFamily:"'Space Mono',monospace",fontSize:11,cursor:"pointer" }}>${amt}</button>)}</div>
      </Card>
      <div style={{ color:"#888",fontSize:10,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:9 }}>WHO'S PLAYING</div>
      <Card style={{ marginBottom:14 }}>{players.map((p:any)=>{const sel=selectedIds.includes(p.id);return<div key={p.id} onClick={()=>setSelectedIds(sel?selectedIds.filter(x=>x!==p.id):[...selectedIds,p.id])} style={{ display:"flex",alignItems:"center",gap:11,padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.05)",cursor:"pointer",opacity:sel?1:0.5 }}><div style={{ width:20,height:20,borderRadius:5,border:`2px solid ${sel?"#C9A84C":"#333"}`,background:sel?"#C9A84C":"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:"#000",fontSize:12,flexShrink:0 }}>{sel?"✓":""}</div><Avatar name={p.name} size={32}/><div style={{ flex:1 }}><div style={{ color:"#fff",fontSize:13 }}>{p.name}</div><div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>{p.wins}W · {p.total_profit>=0?"+":""}${p.total_profit}</div></div></div>;})}
      </Card>
      <button disabled={selectedIds.length<2||loading} onClick={async()=>{setLoading(true);await onStart({selectedIds,sessionBuyIn});setLoading(false);}} style={{ width:"100%",padding:"13px 0",background:selectedIds.length>=2&&!loading?"linear-gradient(135deg,#C9A84C,#E8C56A)":"rgba(255,255,255,0.08)",border:"none",borderRadius:11,color:selectedIds.length>=2&&!loading?"#0A0A0A":"#444",fontFamily:"'Space Mono',monospace",fontWeight:700,fontSize:13,letterSpacing:2,cursor:selectedIds.length>=2&&!loading?"pointer":"not-allowed",display:"flex",alignItems:"center",justifyContent:"center",gap:10 }}>{loading?<Spinner size={16}/>:`START WITH ${selectedIds.length} PLAYERS →`}</button>
    </div>
  );
}
...
function CommSettingsView({ league, players, onBack, onLeagueUpdated, onLeagueDeleted, showToast, showError }:any){
  ...
  const kick=async(id:string,name:string)=>{if(!db||!window.confirm(`Kick ${name}?`))return;await db.from("players").delete().eq("id",id);showToast(`${name} removed.`);onLeagueUpdated(league);};
...
function ProfileTabView({ profile, myLeagues, isSelf, externalName, onFriends, onLogout, onSendFriendRequest }:any){
  ...
  const handleSaveName=async()=>{if(!db||!newName.trim()||newName.trim()===profile.display_name)return;setSavingName(true);const{error}=await db.from("profiles").update({display_name:newName.trim()}).eq("id",profile.id);if(!error){profile.display_name=newName.trim();// Update cache
delete avatarCache[profile.display_name.toLowerCase()];setMsg("Name updated!");setTimeout(()=>setMsg(""),3000);}setSavingName(false);};
  const handleAvatar=async(e:any)=>{const f=e.target.files?.[0];if(!f||!db)return;setUploadingAvatar(true);try{const ext=f.name.split('.').pop();const path=`${profile.id}/avatar.${ext}`;await db.storage.from("avatars").upload(path,f,{upsert:true});const{data:ud}=db.storage.from("avatars").getPublicUrl(path);const url=ud.publicUrl+"?t="+Date.now();await db.from("profiles").update({avatar_url:url}).eq("id",profile.id);profile.avatar_url=url;// Bust avatar cache
avatarCache[profile.display_name.toLowerCase()]=url;setMsg("Photo updated!");setTimeout(()=>setMsg(""),3000);}finally{setUploadingAvatar(false);}};
      ...
      <div style={{ textAlign:"center",marginBottom:18 }}>
        ...
        {isSelf&&editing?(
          <div style={{ marginTop:10,display:"flex",gap:7,justifyContent:"center",alignItems:"center" }}>
            <input value={newName} onChange={e=>setNewName(e.target.value)} style={{ ...inp,width:200,fontSize:15,textAlign:"center" as const,fontFamily:"'Playfair Display',serif" }}/>
            <button onClick={handleSaveName} disabled={savingName||!newName.trim()||newName.trim()===profile.display_name} style={{ padding:"9px 12px",background:"rgba(201,168,76,0.15)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:9,color:"#C9A84C",fontFamily:"'Space Mono',monospace",fontSize:10,cursor:"pointer" }}>{savingName?"...":"✓"}</button>
          </div>
        ):<div style={{ fontFamily:"'Playfair Display',serif",fontSize:22,color:"#fff",marginTop:10 }}>{displayName}</div>}
...
// ─── PLAYER PROFILE (from league standings) ────────────
function PlayerProfileView({ player, profile, onBack, onSendFriendRequest }:any){
  ...
  return(
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      <BackButton onBack={onBack}/>
      <div style={{ textAlign:"center",marginBottom:18 }}>
        <Avatar name={player.name} size={68}/>
        <div style={{ fontFamily:"'Playfair Display',serif",fontSize:22,color:"#fff",marginTop:9 }}>{player.name}</div>
        <div style={{ color:isUp?"#4CAF8C":"#E05555",fontSize:26,fontFamily:"'Space Mono',monospace",fontWeight:700,marginTop:3 }}>{isUp?"+":""}${player.total_profit}</div>
        <div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>in this league</div>
        {!isSelf&&<button onClick={()=>onSendFriendRequest(player.name)} style={{ marginTop:10,padding:"6px 18px",background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:20,color:"#C9A84C",fontFamily:"'Space Mono',monospace",fontSize:10,cursor:"pointer",letterSpacing:1 }}>+ ADD FRIEND</button>}
      </div>
...
export default function HomeGameApp() {
  ...
  type LSV='home'|'joinCreate'|'publicLeagues'|'leagueDetail'|'newSession'|'liveSession'|'commSettings'|'transferComm'|'handRankings'|'playerProfile'|'sessionDetail';
  ...
  useEffect(()=>{
    ...
    db.auth.getSession().then(async({data:{session}})=>{
      if(session?.user){setAuthUser(session.user);setPinnedIds(JSON.parse(localStorage.getItem(`hg_pinned_${session.user.id}`)||'[]'));await init(session.user);}
      else setBootstrapping(false);
    });
    const{data:{subscription}}=db.auth.onAuthStateChange(async(_,session)=>{
      if(session?.user){setAuthUser(session.user);setPinnedIds(JSON.parse(localStorage.getItem(`hg_pinned_${session.user.id}`)||'[]'));await init(session.user);}
      else{setAuthUser(null);setProfile(null);setBootstrapping(false);}
    });
  ...
  const init=async(user:any)=>{
    ...
    if(data){
      ...
      // Cache own avatar immediately
      if(data.avatar_url)avatarCache[data.display_name.toLowerCase()]=data.avatar_url;
      loadMyLeagues(data.display_name,user.id);
    }
  ...
  const loadMyLeagues=async(displayName:string,userId:string)=>{
    ...
    try{
      ...
      if(allIds.length>0){const{data:lgs}=await db.from("leagues").select("id,name,description,code,season,season_length,buy_in,is_public,location_name,commissioner_id,commissioner_name,max_players").in("id",allIds);setMyLeagues((lgs||[]).map((lg:any)=>({...lg,_myUserId:userId})));}
      else setMyLeagues([]);
  ...
  const loadLeagueData=async(lgId:string)=>{
    if(!db)return;
    const[{data:pData},{data:sData},{data:lsData}]=await Promise.all([
      db.from("players").select("*").eq("league_id",lgId).order("total_profit",{ascending:false}),
      db.from("sessions").select("id,pot,winner_name,buy_in_amount,duration_seconds,created_at,chicken_dinner_name,is_live").eq("league_id",lgId).order("created_at",{ascending:false}),
      db.from("sessions").select("*").eq("league_id",lgId).eq("is_live",true).limit(1)
    ]);
  ...
  useEffect(()=>{
    if(!currentLeague||lsv!=='liveSession'||!liveSession)return;
    const t=setInterval(async()=>{const{data}=await db!.from("live_entries").select("*").eq("session_id",liveSession.id);setLiveEntries(data||[]);},10000);
  ...
  const joinLeague=async(lg:any)=>{
    ...
    await loadMyLeagues(profile.display_name,authUser.id);setCurrentLeague({...lg,_myUserId:authUser.id});await loadLeagueData(lg.id);setLsv('leagueDetail');
  };
  ...
  const handleLeave=async()=>{
    ...
    const mp=players.find((p:any)=>p.name.toLowerCase()===profile.display_name.toLowerCase());
    if(mp)await db.from("players").delete().eq("id",mp.id);
  ...
  const handleStartSession=async({selectedIds,sessionBuyIn}:any)=>{
    ...
    await db.from("posts").insert({league_id:currentLeague.id,author_name:profile.display_name,content:`🃏 Game is live! Buy-in: $${sessionBuyIn} · ${sel.length} players. Join from the league tab!`});
  ...
  const handleEndSession=async({entries,elapsed,chickenDinner}:any)=>{
    ...
    try{
      ...
      for(const e of entries){
        ...
        if(isC){const{data:prof}=await db.from("profiles").select("id,opt_in_global,global_total_profit,global_sessions,global_wins,global_time_seconds,chicken_dinners").ilike("display_name",p.name).single();if(prof&&prof.opt_in_global){await db.from("profiles").update({global_total_profit:(prof.global_total_profit||0)+e.profit,global_sessions:(prof.global_sessions||0)+1,global_wins:(prof.global_wins||0)+won,global_time_seconds:(prof.global_time_seconds||0)+elapsed,chicken_dinners:(prof.chicken_dinners||0)+isC}).eq("id",prof.id);}}
      }
  ...
  const sendFriendRequest=async(name:string)=>{if(!db||!profile)return;const{error}=await db.from("friends").insert({requester_name:profile.display_name,recipient_name:name,status:"pending"});if(error)showError("Already sent or friends!");else showToast(`Request sent to ${name}!`);};
  ...
  const renderLeague=()=>{
    if(lsv==='joinCreate')return<JoinCreateView profile={profile} loading={loadingLeagues} onBack={()=>setLsv('home')} onEnter={handleJoinCreate}/>;
    ...
    if(lsv==='leagueDetail'&&currentLeague)return<LeagueDetailView league={currentLeague} players={players} sessions={sessions} profile={profile} isCommissioner={isComm} onViewPlayer={(p:any)=>{setSelectedPlayer(p);setLsv('playerProfile');}} onStartSession={()=>liveSession?setLsv('liveSession'):setLsv('newSession')} onBack={()=>{setCurrentLeague(null);setLsv('home');}} onCommSettings={()=>setLsv('commSettings')} liveSession={liveSession} onLeaveLeague={handleLeave} onViewHandRankings={()=>setLsv('handRankings')} onViewSession={(s:any)=>{setSelectedSession(s);setLsv('sessionDetail');}}/>;
    if(lsv==='sessionDetail'&&selectedSession&&currentLeague)return<SessionDetailView session={selectedSession} league={currentLeague} players={players} isCommissioner={isComm} onBack={()=>setLsv('leagueDetail')} onSaved={()=>loadLeagueData(currentLeague.id)} showToast={showToast}/>;
    ...
    if(lsv==='newSession'&&currentLeague)return<NewSessionView league={currentLeague} players={players} onStart={handleStartSession} onBack={()=>setLsv('leagueDetail')}/>;
