...
const VIEWS = {
  ...
  handRankings:'handRankings', friendStats:'friendStats', transferComm:'transferComm'
};
...
function Spinner({ size=24 }:any) {
  return <div style={{ width:size,height:size,border:`2px solid rgba(201,168,76,0.2)`,borderTopColor:"#C9A84C",borderRadius:"50%",animation:"spin 0.7s linear infinite",flexShrink:0 }}/>;
}
    ...
    <div style={{ minHeight:"100vh",background:"#0A0A0A",display:"flex",alignItems:"center",justifyContent:"center" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ position:"relative",width:64,height:64,margin:"0 auto 20px" }}>
          <div style={{ position:"absolute",inset:0,border:"2px solid rgba(201,168,76,0.15)",borderTopColor:"#C9A84C",borderRadius:"50%",animation:"spin 0.7s linear infinite" }}/>
          <div style={{ position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,color:"#C9A84C" }}>♠</div>
        </div>
        <div style={{ color:"#333",fontFamily:"'Space Mono',monospace",fontSize:11,letterSpacing:2 }}>LOADING</div>
      </div>
    </div>
  );
}
function Toggle({ value, onChange, label, sub }:any) {
  return <div onClick={()=>onChange(!value)} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"13px 14px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,cursor:"pointer",marginBottom:10 }}>
    <div style={{ flex:1,paddingRight:12 }}><div style={{ color:"#fff",fontSize:13 }}>{label}</div>{sub&&<div style={{ color:"#555",fontSize:11,marginTop:2 }}>{sub}</div>}</div>
...
function AuthView() {
  ...
  const handle=async()=>{
    ...
    else if(tab==='signup'){if(pw.length<6){setErr("Password must be at least 6 characters.");setLoading(false);return;}const{error}=await db.auth.signUp({email,password:pw});if(error)setErr(error.message);else setMsg("Account created! Sign in now.");}
...
function MyLeaguesView({ profile, myLeagues, loading, onSelectLeague, onJoinCreate, onFriends, onSettings, onFeed, onPublicLeagues, onMyAccount, onGlobalLeaderboard }:any) {
  return (
    <div style={{ minHeight:"100vh",background:"#0A0A0A",padding:"20px 16px 100px",maxWidth:500,margin:"0 auto" }}>
      ...
      {!loading&&myLeagues.length===0&&<Card style={{ marginBottom:14,textAlign:"center" }}><div style={{ padding:"20px 0" }}><div style={{ fontSize:32,marginBottom:10 }}>♠</div><div style={{ color:"#555",fontFamily:"'Space Mono',monospace",fontSize:12 }}>No leagues yet — join or create one</div></div></Card>}
            ...
            {tab==="join"?<div style={{ marginBottom:20 }}><label style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:1.5,display:"block",marginBottom:8 }}>INVITE CODE</label><input value={code} onChange={e=>setCode(e.target.value.toUpperCase())} placeholder="e.g. FNP2026" style={{ ...inp,color:"#C9A84C",fontSize:20,letterSpacing:4,textAlign:"center" as const }}/></div>:(
              <>
                ...
                <div style={{ display:"flex",gap:10,marginBottom:11 }}>
                  ...
                  <div style={{ flex:1 }}><label style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:1.5,display:"block",marginBottom:7 }}>SEASON LENGTH</label><input type="number" value={seasonLength} onChange={e=>setSeasonLength(e.target.value)} placeholder="0=unlimited" style={inp}/></div>
                </div>
                <div style={{ marginBottom:11 }}>
                  <label style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:1.5,display:"block",marginBottom:7 }}>MAX PLAYERS</label>
                  <div style={{ display:"flex",gap:6 }}>{MAX_PLAYER_OPTIONS.map(n=><button key={n} onClick={()=>setMaxPlayers(n)} style={{ flex:1,padding:"9px 0",borderRadius:10,background:maxPlayers===n?"rgba(201,168,76,0.2)":"rgba(255,255,255,0.04)",border:`1px solid ${maxPlayers===n?"rgba(201,168,76,0.4)":"rgba(255,255,255,0.08)"}`,color:maxPlayers===n?"#C9A84C":"#555",fontFamily:"'Space Mono',monospace",fontSize:12,cursor:"pointer" }}>{n}</button>)}</div>
                  <div style={{ color:"#444",fontSize:11,marginTop:5 }}>Default is 12 · max 50</div>
                </div>
                <div style={{ marginBottom:11 }}>
                  ...
                  <div style={{ display:"flex",gap:8 }}><input value={locationName} onChange={e=>setLocationName(e.target.value)} placeholder="e.g. San Francisco, CA" style={{ ...inp,flex:1 }}/><button onClick={detectLocation} style={{ padding:"0 14px",background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:10,color:"#C9A84C",fontFamily:"'Space Mono',monospace",fontSize:11,cursor:"pointer",flexShrink:0 }}>{detecting?<Spinner size={14}/>:"📍"}</button></div>
                </div>
                <Toggle value={isPublic} onChange={setIsPublic} label="Public League 🌍" sub="Anyone can find & join without a code"/>
              </>
            )}
            <button onClick={()=>canSubmit&&!loading&&onEnter({tab,code,leagueName,description,buyIn:Number(buyIn),season,seasonLength:Number(seasonLength),isPublic,locationName,maxPlayers})} style={{ width:"100%",padding:"14px 0",marginTop:8,background:canSubmit&&!loading?"linear-gradient(135deg,#C9A84C,#E8C56A)":"rgba(255,255,255,0.08)",border:"none",borderRadius:12,color:canSubmit&&!loading?"#0A0A0A":"#444",fontFamily:"'Space Mono',monospace",fontWeight:700,fontSize:14,letterSpacing:2,cursor:canSubmit&&!loading?"pointer":"not-allowed",display:"flex",alignItems:"center",justifyContent:"center",gap:10 }}>{loading?<Spinner size={18}/>:tab==="join"?"Join League →":"Create League →"}</button>
          </div>
        </Card>
      </div>
    </div>
...
function PublicLeaguesView({ profile, onBack, onJoin }:any) {
  ...
  const loadPublic=async()=>{if(!db)return;setLoading(true);const{data}=await db.from("leagues").select("*").eq("is_public",true).order("created_at",{ascending:false});setLeagues(data||[]);setLoading(false);};
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <div style={{ display:"flex",gap:8,marginBottom:12 }}>
        ...
        <button onClick={detectLocation} style={{ padding:"0 14px",background:"rgba(85,119,204,0.1)",border:"1px solid rgba(85,119,204,0.3)",borderRadius:10,color:"#5577CC",fontFamily:"'Space Mono',monospace",fontSize:11,cursor:"pointer",flexShrink:0 }}>{detecting?<Spinner size={14}/>:"📍"}</button>
      </div>
        ...
        <Card key={lg.id} style={{ marginBottom:12 }}>
          <div style={{ display:"flex",gap:12,marginBottom:12 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ color:"#555",fontSize:11,fontFamily:"'Space Mono',monospace",marginTop:4 }}>{lg.season} · ${lg.buy_in}{lg.location_name?<span style={{ color:"#5577CC" }}> · 📍{lg.location_name}</span>:null} · {lg.max_players||12} max</div>
            </div>
          </div>
...
function GlobalLeaderboardView({ profile, onBack }:any) {
  ...
  const loadLeaders=async()=>{if(!db)return;setLoading(true);const{data}=await db.from("profiles").select("*").eq("opt_in_global",true);setLeaders(data||[]);const{data:me}=await db.from("profiles").select("*").eq("id",profile.id).single();setMyStats(me);setLoading(false);};
  const myHours=myStats?Math.floor((myStats.global_time_seconds||0)/3600):0;
  const canPlace=myHours>=HOURS_REQUIRED_GLOBAL;
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <Card style={{ marginBottom:16,background:"rgba(201,168,76,0.05)",border:"1px solid rgba(201,168,76,0.2)" }}>
        ...
        <div style={{ color:"#888",fontSize:12,lineHeight:1.7 }}>Opt in from <span style={{ color:"#fff" }}>Settings</span> to appear here. You need <span style={{ color:"#C9A84C" }}>{HOURS_REQUIRED_GLOBAL} hours</span> of logged league play to place on the board. Anyone can view it.</div>
        {myStats&&<div style={{ marginTop:10,paddingTop:10,borderTop:"1px solid rgba(201,168,76,0.15)" }}><div style={{ color:"#555",fontSize:11,fontFamily:"'Space Mono',monospace" }}>YOUR TIME: <span style={{ color:myHours>=HOURS_REQUIRED_GLOBAL?"#4CAF8C":"#E05555" }}>{myHours}/{HOURS_REQUIRED_GLOBAL}h</span>{myHours<HOURS_REQUIRED_GLOBAL&&" — keep grinding!"}</div>{!myStats.opt_in_global&&canPlace&&<div style={{ color:"#C9A84C",fontSize:11,marginTop:4 }}>You qualify! Enable in Settings.</div>}</div>}
      </Card>
      <div style={{ display:"flex",gap:6,marginBottom:14 }}>
        {([['profit','$ PROFIT'],['winpct','WIN %'],['time','TIME']] as any[]).map(([k,l])=><button key={k} onClick={()=>setSortBy(k)} style={{ flex:1,padding:"8px 0",borderRadius:20,background:sortBy===k?"rgba(201,168,76,0.2)":"rgba(255,255,255,0.04)",border:`1px solid ${sortBy===k?"rgba(201,168,76,0.4)":"rgba(255,255,255,0.08)"}`,color:sortBy===k?"#C9A84C":"#555",fontFamily:"'Space Mono',monospace",fontSize:10,cursor:"pointer",letterSpacing:1 }}>{l}</button>)}
      </div>
      {loading&&<div style={{ display:"flex",justifyContent:"center",padding:40 }}><Spinner/></div>}
      {!loading&&sorted.length===0&&<Card><div style={{ textAlign:"center",padding:"28px 0",color:"#555",fontFamily:"'Space Mono',monospace",fontSize:12 }}>No one on the board yet — opt in from Settings!</div></Card>}
      {sorted.map((p:any,i:number)=>{
        const isMe=p.id===profile.id;const medals=["🥇","🥈","🥉"];const winPct=p.global_sessions>0?((p.global_wins/p.global_sessions)*100).toFixed(0):0;const hrs=Math.floor((p.global_time_seconds||0)/3600);
        return <div key={p.id} style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 14px",marginBottom:8,background:isMe?"rgba(201,168,76,0.06)":"rgba(255,255,255,0.02)",border:`1px solid ${isMe?"rgba(201,168,76,0.3)":"rgba(255,255,255,0.06)"}`,borderRadius:13 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ color:"#fff",fontSize:14 }}>{p.display_name}{isMe&&<span style={{ color:"#C9A84C",fontSize:10,fontFamily:"'Space Mono',monospace" }}> (you)</span>}</div>
            <div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>{p.global_sessions||0} games · {winPct}% win · {hrs}h played</div>
          </div>
          <div style={{ textAlign:"right" }}>
            ...
            {p.chicken_dinners>0&&<div style={{ color:"#C9A84C",fontSize:10 }}>🍗×{p.chicken_dinners}</div>}
          </div>
        </div>;
      })}
    </div>
...
// ─── LEAGUE VIEW (Clash Royale style) ──────────────────
function LeagueView({ league, players, sessions, profile, isCommissioner, onNav, onStartSession, onBack, onCommSettings, liveSession, onLeaveLeague }:any) {
    ...
    <div style={{ maxWidth:500,margin:"0 auto" }}>
      ...
      <div style={{ padding:"0 16px 20px" }}>
        ...
        <Card style={{ padding:0,overflow:"hidden",marginBottom:14 }}>
          ...
          {sorted.map((p:any,i:number)=>{
            const isComm=p.name.toLowerCase()===league.commissioner_name?.toLowerCase();const winPct=p.session_count>0?((p.wins/p.session_count)*100).toFixed(0):0;const medals=["🥇","🥈","🥉"];const isTop3=i<3;
            return <div key={p.id} onClick={()=>onNav(VIEWS.profile,p)} style={{ display:"flex",alignItems:"center",gap:12,padding:"13px 18px",borderBottom:"1px solid rgba(255,255,255,0.05)",cursor:"pointer",background:i===0?"rgba(201,168,76,0.04)":"transparent" }}>
              ...
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ color:"#fff",fontSize:14,display:"flex",alignItems:"center",gap:5 }}>{p.name.length>14?p.name.slice(0,14)+"…":p.name} {isComm&&<span>👑</span>} {p.streak>1&&<span>🔥</span>} {p.chicken_dinners>0&&<span>🍗</span>}</div>
...
function TransferCommView({ league, players, profile, onBack, onTransferred }:any) {
  ...
  const handleTransfer=async()=>{
    if(!db||!selected)return;setLoading(true);
    const newComm=players.find((p:any)=>p.name===selected);if(!newComm)return;
    // Find the profile of the new commissioner by name
    const{data:newProf}=await db.from("profiles").select("id").ilike("display_name",selected).limit(1);
    const newCommId=newProf?.[0]?.id||null;
    await db.from("leagues").update({commissioner_name:selected,commissioner_id:newCommId}).eq("id",league.id);
    onTransferred();
  };
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      <BackHeader onBack={onBack} sub="BEFORE YOU GO" title="Transfer Commissioner"/>
      <div style={{ color:"#888",fontSize:12,fontFamily:"'Space Mono',monospace",lineHeight:1.7,marginBottom:20 }}>You're the commissioner. Before leaving, choose who takes over. They'll have full control of this league.</div>
      <Card style={{ marginBottom:16 }}>
        {others.length===0&&<div style={{ color:"#555",fontFamily:"'Space Mono',monospace",fontSize:12,textAlign:"center",padding:"16px 0" }}>No other players to transfer to.</div>}
        {others.map((p:any)=><div key={p.id} onClick={()=>setSelected(p.name)} style={{ display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:"1px solid rgba(255,255,255,0.05)",cursor:"pointer" }}>
          <div style={{ width:22,height:22,borderRadius:6,border:`2px solid ${selected===p.name?"#C9A84C":"#333"}`,background:selected===p.name?"#C9A84C":"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:"#000",fontSize:13,flexShrink:0 }}>{selected===p.name?"✓":""}</div>
          <Avatar name={p.name} size={36}/><div style={{ flex:1 }}><div style={{ color:"#fff" }}>{p.name}</div><div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>{p.session_count} sessions</div></div>
        </div>)}
      </Card>
...
// ─── LIVE SESSION (all players see + enter stats) ──────
function LiveSessionView({ session, liveEntries, players, profile, isCommissioner, league, onBack, onSubmitEntry, onEndSession }:any) {
  ...
  const handleSubmitMyStats=async()=>{if(!myPlayer)return;setSaving(true);await onSubmitEntry({player_id:myPlayer.id,player_name:profile.display_name,buy_in:myBuyIn,rebuys:myRebuys});setSaving(false);};
  ...
  return (
    <div style={{ padding:"16px 16px",maxWidth:500,margin:"0 auto" }}>
      <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:14 }}>
        <button onClick={onBack} style={{ background:"none",border:"none",color:"#555",fontSize:22,cursor:"pointer" }}>←</button>
        <div style={{ flex:1 }}><div style={{ color:"#4CAF8C",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2 }}>● GAME IS LIVE</div><div style={{ fontFamily:"'Playfair Display',serif",fontSize:20,color:"#fff" }}>{league.name}</div></div>
      </div>
      ...
      {/* Anyone enters their own stats */}
      {myPlayer&&<Card style={{ marginBottom:14,border:"1px solid rgba(201,168,76,0.3)",background:"rgba(201,168,76,0.04)" }}>
        <div style={{ color:"#C9A84C",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:12 }}>YOUR STATS</div>
        <div style={{ display:"flex",gap:12,marginBottom:12 }}>
          <div style={{ flex:1 }}><div style={{ color:"#888",fontSize:10,fontFamily:"'Space Mono',monospace",marginBottom:5 }}>BUY-IN ($)</div><input type="number" value={myBuyIn||""} onChange={e=>setMyBuyIn(Number(e.target.value))} style={{...ni,width:"100%"}}/></div>
          <div style={{ flex:1 }}><div style={{ color:"#888",fontSize:10,fontFamily:"'Space Mono',monospace",marginBottom:5 }}>REBUYS ($)</div><input type="number" value={myRebuys||""} onChange={e=>setMyRebuys(Number(e.target.value))} style={{...ni,width:"100%"}}/></div>
        </div>
        <button onClick={handleSubmitMyStats} disabled={saving} style={{ width:"100%",padding:"10px 0",background:"rgba(201,168,76,0.15)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:10,color:"#C9A84C",fontFamily:"'Space Mono',monospace",fontSize:12,letterSpacing:1.5,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8 }}>{saving?<Spinner size={14}/>:"SUBMIT MY STATS ✓"}</button>
      </Card>}
      <Card style={{ marginBottom:14 }}>
        <div style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:12 }}>IN SESSION ({liveEntries.length})</div>
        {liveEntries.length===0&&<div style={{ color:"#555",fontSize:12,fontFamily:"'Space Mono',monospace",textAlign:"center",padding:"10px 0" }}>Waiting for players...</div>}
      ...
      {/* Commissioner ends session */}
      {isCommissioner&&<>
        {!showCashout&&<button onClick={()=>setShowCashout(true)} style={{ width:"100%",padding:"13px 0",background:"linear-gradient(135deg,#5a0000,#8B1A1A)",border:"1px solid rgba(224,85,85,0.4)",borderRadius:12,color:"#E05555",fontFamily:"'Space Mono',monospace",fontWeight:700,fontSize:13,letterSpacing:2,cursor:"pointer" }}>END GAME & ENTER CASH-OUTS →</button>}
        {showCashout&&<Card>
          <div style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:12 }}>CASH-OUTS</div>
          {liveEntries.map((e:any)=><div key={e.player_id} style={{ display:"flex",alignItems:"center",gap:10,marginBottom:10 }}><Avatar name={e.player_name} size={28}/><div style={{ flex:1 }}><div style={{ color:"#fff",fontSize:13 }}>{e.player_name}</div><div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>in: ${(e.buy_in||0)+(e.rebuys||0)}</div></div><input type="number" value={cashOuts[e.player_id]||""} onChange={ev=>setCashOuts((c:any)=>({...c,[e.player_id]:Number(ev.target.value)}))} placeholder="$0" style={{...ni,width:"75px"}}/></div>)}
          <div style={{ marginTop:12,marginBottom:4 }}>
            <div style={{ color:"#C9A84C",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:8 }}>🍗 CHICKEN DINNER (biggest winner)</div>
            <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>{liveEntries.map((e:any)=><button key={e.player_id} onClick={()=>setChickenDinner(e.player_name)} style={{ padding:"6px 12px",borderRadius:20,background:chickenDinner===e.player_name?"rgba(201,168,76,0.2)":"rgba(255,255,255,0.04)",border:`1px solid ${chickenDinner===e.player_name?"rgba(201,168,76,0.4)":"rgba(255,255,255,0.08)"}`,color:chickenDinner===e.player_name?"#C9A84C":"#555",fontFamily:"'Space Mono',monospace",fontSize:11,cursor:"pointer" }}>{e.player_name}</button>)}</div>
          </div>
      ...
      {!isCommissioner&&<div style={{ padding:"12px 14px",background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,color:"#444",fontFamily:"'Space Mono',monospace",fontSize:11,textAlign:"center" }}>👑 Commissioner will end the session and enter cash-outs</div>}
    </div>
  );
}
...
function NewSessionView({ league, players, onStart, onBack }:any) {
  ...
  const handleStart=async()=>{if(selectedIds.length<2||!db)return;setLoading(true);await onStart({selectedIds,sessionBuyIn});setLoading(false);};
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      <BackHeader onBack={onBack} sub="NEW SESSION" title="Tonight's Setup"/>
      <Card style={{ marginBottom:14 }}>
        <div style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:10 }}>BUY-IN FOR TONIGHT</div>
        <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:10 }}>
          ...
          <div style={{ flex:1,textAlign:"center" }}><div style={{ color:"#C9A84C",fontSize:32,fontFamily:"'Space Mono',monospace",fontWeight:700 }}>${sessionBuyIn}</div></div>
      ...
      <Card style={{ marginBottom:16 }}>
        {players.map((p:any)=>{const sel=selectedIds.includes(p.id);return <div key={p.id} onClick={()=>setSelectedIds(sel?selectedIds.filter(x=>x!==p.id):[...selectedIds,p.id])} style={{ display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:"1px solid rgba(255,255,255,0.05)",cursor:"pointer",opacity:sel?1:0.5 }}>
          ...
          <Avatar name={p.name} size={34}/><div style={{ flex:1 }}><div style={{ color:"#fff" }}>{p.name}</div><div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>{p.wins}W · {p.total_profit>=0?"+":""}${p.total_profit}</div></div>
        </div>;})}
      </Card>
...
function HandRankingsView({ onBack }:any) {
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <div style={{ color:"#555",fontSize:12,fontFamily:"'Space Mono',monospace",marginBottom:16 }}>#1 best → #10 lowest</div>
      {HAND_RANKINGS.map((h,i)=>(
        <div key={h.rank} style={{ display:"flex",alignItems:"center",gap:14,padding:"12px 15px",marginBottom:7,background:"rgba(255,255,255,0.03)",border:`1px solid ${h.rank<=2?"rgba(201,168,76,0.25)":"rgba(255,255,255,0.07)"}`,borderRadius:13 }}>
          <div style={{ width:30,height:30,borderRadius:9,background:`${h.color}22`,border:`1px solid ${h.color}44`,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Space Mono',monospace",fontWeight:700,color:h.color,fontSize:12,flexShrink:0 }}>{h.rank}</div>
          <div style={{ flex:1 }}>
            <div style={{ color:h.color,fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700 }}>{h.name}</div>
            <div style={{ color:"#666",fontSize:11,marginTop:1 }}>{h.desc}</div>
            <div style={{ color:"#444",fontSize:10,fontFamily:"'Space Mono',monospace",marginTop:2,letterSpacing:1 }}>{h.example}</div>
          </div>
          {h.rank<=3&&<div style={{ fontSize:16,flexShrink:0 }}>{["👑","⭐","💎"][i]}</div>}
        </div>
      ))}
      <Card style={{ marginTop:14,background:"rgba(201,168,76,0.04)" }}>
        <div style={{ color:"#C9A84C",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:8 }}>QUICK TIP</div>
        <div style={{ color:"#888",fontSize:12,lineHeight:1.7 }}>In Texas Hold'em, use the best 5 cards from your 2 hole cards + 5 community cards. Ties broken by highest card.</div>
      </Card>
    </div>
  );
}

// ─── FEED (mutual friends only, anyone can post live game) ─
function FeedView({ profile, myLeagues, currentLeague, onBack }:any) {
  ...
  const fileRef=useRef<HTMLInputElement>(null);const [editingPost,setEditingPost]=useState<string|null>(null);const [editContent,setEditContent]=useState("");
  useEffect(()=>{loadPosts();},[]);
  ...
  const handlePost=async()=>{
    ...
    await db.from("posts").insert({league_id:selectedLeagueId,author_name:profile.display_name,content:newPost.trim()||null,media_url:mediaUrl,media_type:mediaType});setNewPost("");setMediaFile(null);setMediaPreview(null);await loadPosts();}finally{setUploading(false);}
  };
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <div style={{ color:"#555",fontSize:11,fontFamily:"'Space Mono',monospace",marginBottom:14 }}>Posts from you + mutual friends · your leagues only</div>
      {myLeagues.length>0&&<Card style={{ marginBottom:16 }}>
        <div style={{ display:"flex",gap:10,marginBottom:10 }}><Avatar name={profile.display_name} url={profile.avatar_url} size={32}/><textarea value={newPost} onChange={e=>setNewPost(e.target.value)} placeholder="Share a moment, start a live game post..." style={{ flex:1,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:10,padding:10,color:"#fff",fontFamily:"'Space Mono',monospace",fontSize:12,resize:"none",height:62,outline:"none" }}/></div>
        ...
        <div style={{ display:"flex",gap:8,alignItems:"center" }}>
          <button onClick={()=>fileRef.current?.click()} style={{ padding:"6px 12px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,color:"#888",fontFamily:"'Space Mono',monospace",fontSize:10,cursor:"pointer" }}>📷</button>
      ...
      {loading&&<div style={{ display:"flex",justifyContent:"center",padding:40 }}><Spinner/></div>}
      {!loading&&posts.length===0&&<Card><div style={{ textAlign:"center",padding:"26px 0",color:"#555",fontFamily:"'Space Mono',monospace",fontSize:12 }}>No posts yet.<br/>Add friends from league standings to see their posts!</div></Card>}
      {posts.map((post:any)=>{
        const isMine=post.author_name.toLowerCase()===profile.display_name.toLowerCase();
        const isComm=isCommForLeague(post.league_id);
        const canEdit=isMine||isComm;
        return <Card key={post.id} style={{ marginBottom:12 }}>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:10 }}>
            <Avatar name={post.author_name} size={32}/>
            <div style={{ flex:1 }}><div style={{ color:"#fff",fontWeight:600 }}>{post.author_name}</div><div style={{ display:"flex",gap:8,alignItems:"center",marginTop:2 }}><span style={{ color:"#C9A84C",fontSize:10,fontFamily:"'Space Mono',monospace",background:"rgba(201,168,76,0.1)",padding:"1px 7px",borderRadius:10 }}>{getLeagueName(post.league_id)}</span><span style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>{timeAgo(post.created_at)}</span></div></div>
            {canEdit&&<div style={{ display:"flex",gap:6 }}>
              {isMine&&<button onClick={()=>{setEditingPost(post.id);setEditContent(post.content||"");}} style={{ background:"none",border:"none",color:"#555",fontSize:12,cursor:"pointer",fontFamily:"'Space Mono',monospace" }}>EDIT</button>}
              <button onClick={()=>handleDelete(post.id)} style={{ background:"none",border:"none",color:"#E05555",fontSize:12,cursor:"pointer",fontFamily:"'Space Mono',monospace" }}>DEL</button>
            </div>}
          </div>
...
function StandingsView({ players, league, onNav, onBack }:any) {
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <div style={{ display:"flex",gap:6,marginBottom:12,flexWrap:"wrap" }}>{[["profit","Profit"],["wins","Wins"],["sessions","Sessions"],["best","Best Night"]].map(([key,label])=><button key={key} onClick={()=>setSortBy(key)} style={{ padding:"5px 11px",borderRadius:20,background:sortBy===key?"rgba(201,168,76,0.2)":"rgba(255,255,255,0.04)",border:`1px solid ${sortBy===key?"rgba(201,168,76,0.4)":"rgba(255,255,255,0.08)"}`,color:sortBy===key?"#C9A84C":"#555",fontSize:11,fontFamily:"'Space Mono',monospace",cursor:"pointer" }}>{label}</button>)}</div>
      <Card>{sorted.length===0&&<div style={{ color:"#555",fontFamily:"'Space Mono',monospace",fontSize:12,textAlign:"center",padding:"16px 0" }}>No data yet</div>}{sorted.map((p:any,i:number)=>{const isComm=p.name.toLowerCase()===league.commissioner_name?.toLowerCase();return <div key={p.id} onClick={()=>onNav(VIEWS.profile,p)} style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:i<sorted.length-1?"1px solid rgba(255,255,255,0.05)":"none",cursor:"pointer" }}><div style={{ color:i<3?"#C9A84C":"#444",fontFamily:"'Space Mono',monospace",fontSize:13,width:18 }}>{i+1}</div><Avatar name={p.name} size={36}/><div style={{ flex:1 }}><div style={{ color:"#fff",display:"flex",alignItems:"center",gap:5 }}>{p.name} {isComm&&<span>👑</span>} {p.streak>1&&<span>🔥</span>} {p.chicken_dinners>0&&<span>🍗</span>}</div><div style={{ color:"#555",fontSize:10,fontFamily:"'Space Mono',monospace" }}>{p.session_count} sessions · {p.wins}W</div></div><div style={{ textAlign:"right" }}><div style={{ color:p.total_profit>=0?"#4CAF8C":"#E05555",fontFamily:"'Space Mono',monospace",fontWeight:700,fontSize:13 }}>{p.total_profit>=0?"+":""}${p.total_profit}</div>{p.session_count>0&&<div style={{ color:"#444",fontSize:10 }}>{((p.wins/p.session_count)*100).toFixed(0)}% win</div>}</div></div>;})}
      </Card>
    </div>
  );
}
...
function ProfileView({ player, profile, onBack, onSendFriendRequest }:any) {
  ...
  const timeStr=player.time_played_seconds>0?fmtSeconds(player.time_played_seconds):"—";
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      <button onClick={onBack} style={{ background:"none",border:"none",color:"#555",fontSize:22,cursor:"pointer",marginBottom:14 }}>←</button>
      ...
      <div style={{ display:"flex",gap:8,marginBottom:14 }}><StatBox label="Sessions" value={player.session_count}/><StatBox label="Wins" value={player.wins} accent="#4CAF8C"/><StatBox label="Win %" value={`${winRate}%`} accent="#5577CC"/><StatBox label="Best" value={`$${player.best_night}`}/></div>
...
function MyAccountView({ profile, myLeagues, onBack }:any) {
  ...
  const loadStats=async()=>{
    ...
    if(!rows||rows.length===0){setAllStats({total_profit:0,sessions:0,wins:0,best_night:0,leagues:0,time_seconds:0,chicken_dinners:0});setLoading(false);return;}
    ...
    const losses=sessions-wins;const avg=sessions>0?(total_profit/sessions):0;
    setAllStats({total_profit,sessions,wins,losses,best_night,leagues:rows.length,time_seconds,chicken_dinners,avg});
    setLoading(false);
  };
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <div style={{ textAlign:"center",marginBottom:24 }}>
        <Avatar name={profile.display_name} url={profile.avatar_url} size={76}/>
        <div style={{ fontFamily:"'Playfair Display',serif",fontSize:24,color:"#fff",marginTop:10 }}>{profile.display_name}</div>
        <div style={{ color:"#555",fontSize:11,fontFamily:"'Space Mono',monospace",marginTop:4 }}>{profile.email}</div>
        {!loading&&allStats&&<><div style={{ color:isUp?"#4CAF8C":"#E05555",fontSize:36,fontFamily:"'Space Mono',monospace",fontWeight:700,marginTop:14 }}>{isUp?"+":""}${allStats.total_profit}</div><div style={{ color:"#555",fontSize:11,fontFamily:"'Space Mono',monospace" }}>all-time profit · {allStats.leagues} leagues</div></>}
      ...
      {!loading&&allStats&&<>
        ...
        <Card style={{ marginBottom:14 }}>
          <div style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:12 }}>BREAKDOWN</div>
          {([["Total profit / loss",`${isUp?"+":""}$${allStats.total_profit}`,isUp?"#4CAF8C":"#E05555"],["Sessions won",`${allStats.wins}`,  "#4CAF8C"],["Sessions lost",`${allStats.losses}`,"#E05555"],["Biggest single win",`$${allStats.best_night}`,"#C9A84C"],["Chicken dinners 🍗",`${allStats.chicken_dinners}`,"#C9A84C"],["Total time at the table",allStats.time_seconds>0?fmtSeconds(allStats.time_seconds):"—","#888"]] as any[]).map(([label,val,col]:any,i:number,arr:any[])=><div key={label} style={{ display:"flex",justifyContent:"space-between",padding:"9px 0",borderBottom:i<arr.length-1?"1px solid rgba(255,255,255,0.05)":"none" }}><span style={{ color:"#555",fontFamily:"'Space Mono',monospace",fontSize:12 }}>{label}</span><span style={{ color:col,fontFamily:"'Space Mono',monospace",fontWeight:700 }}>{val}</span></div>)}
        </Card>
        <Card>
          <div style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:12 }}>MY LEAGUES</div>
          {myLeagues.length===0&&<div style={{ color:"#555",fontSize:12,fontFamily:"'Space Mono',monospace" }}>No leagues yet.</div>}
...
function FriendsView({ profile, onBack, onViewFriendStats }:any) {
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <div><div style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:10 }}>FRIENDS ({friends.length})</div>{!loading&&friends.length===0&&<Card><div style={{ textAlign:"center",padding:"22px 0",color:"#555",fontFamily:"'Space Mono',monospace",fontSize:12 }}>No friends yet — tap players in standings!</div></Card>}{friends.map((f:any)=>{const name=getFriendName(f);return <Card key={f.id} style={{ marginBottom:10,display:"flex",alignItems:"center",gap:12,cursor:"pointer" }} onClick={()=>onViewFriendStats(name)}><Avatar name={name} size={40}/><div style={{ flex:1 }}><div style={{ color:"#fff" }}>{name}</div><div style={{ color:"#4CAF8C",fontSize:11,fontFamily:"'Space Mono',monospace" }}>● Friends · tap for stats</div></div><span style={{ color:"#555",fontSize:18 }}>›</span></Card>;})}</div>
    </div>
  );
}
...
function FriendStatsView({ friendName, onBack }:any) {
  ...
  useEffect(()=>{if(!db)return;(async()=>{setLoading(true);const{data:rows}=await db.from("players").select("*").ilike("name",friendName);if(!rows||rows.length===0){setStats(null);setLoading(false);return;}setStats({total_profit:rows.reduce((a:number,p:any)=>a+(p.total_profit||0),0),sessions:rows.reduce((a:number,p:any)=>a+(p.session_count||0),0),wins:rows.reduce((a:number,p:any)=>a+(p.wins||0),0),best_night:Math.max(0,...rows.map((p:any)=>p.best_night||0)),leagues:rows.length,time_seconds:rows.reduce((a:number,p:any)=>a+(p.time_played_seconds||0),0),chicken_dinners:rows.reduce((a:number,p:any)=>a+(p.chicken_dinners||0),0)});setLoading(false);})();},[friendName]);
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      {!loading&&stats&&<><div style={{ textAlign:"center",marginBottom:22 }}><Avatar name={friendName} size={68}/><div style={{ fontFamily:"'Playfair Display',serif",fontSize:22,color:"#fff",marginTop:10 }}>{friendName}</div><div style={{ color:isUp?"#4CAF8C":"#E05555",fontSize:32,fontFamily:"'Space Mono',monospace",fontWeight:700,marginTop:8 }}>{isUp?"+":""}${stats.total_profit}</div><div style={{ color:"#555",fontSize:11,fontFamily:"'Space Mono',monospace" }}>all-time · {stats.leagues} leagues</div></div><div style={{ display:"flex",gap:8,marginBottom:12 }}><StatBox label="Sessions" value={stats.sessions}/><StatBox label="Wins" value={stats.wins} accent="#4CAF8C"/><StatBox label="Win %" value={stats.sessions>0?`${((stats.wins/stats.sessions)*100).toFixed(0)}%`:"—"} accent="#5577CC"/><StatBox label="Best" value={`$${stats.best_night}`}/></div><div style={{ display:"flex",gap:8,marginBottom:14 }}><StatBox label="Time Played" value={stats.time_seconds>0?fmtSeconds(stats.time_seconds):"—"} accent="#888"/><StatBox label="🍗 Dinners" value={stats.chicken_dinners} accent="#C9A84C"/></div></>}
    </div>
  );
}
...
function CommSettingsView({ league, players, onBack, onLeagueUpdated, onLeagueDeleted, showToast, showError }:any) {
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <Card style={{ marginBottom:12 }}>
        <div style={{ color:"#888",fontSize:11,fontFamily:"'Space Mono',monospace",letterSpacing:2,marginBottom:12 }}>PLAYERS ({players.length}/{maxPlayers})</div>
...
function SettingsView({ profile, onBack, onLogout, onProfileUpdated }:any) {
  ...
  const handleOptIn=async(val:boolean)=>{if(!db)return;setOptIn(val);await db.from("profiles").update({opt_in_global:val}).eq("id",profile.id);onProfileUpdated({...profile,opt_in_global:val});setMsg(val?"You're on the global leaderboard!":"Removed from leaderboard.");setTimeout(()=>setMsg(""),3000);};
  ...
  return (
    <div style={{ padding:"20px 16px",maxWidth:500,margin:"0 auto" }}>
      ...
      <div style={{ textAlign:"center",marginBottom:20 }}>
        ...
        <div style={{ color:"#555",fontSize:11,fontFamily:"'Space Mono',monospace",marginTop:8 }}>Tap camera to update</div>
      </div>
      <Card style={{ marginBottom:12 }}>
...
function NavBar({ view, onNav, onHandRankings }:any) {
  const inLeague=[VIEWS.league,VIEWS.feed,VIEWS.standings,VIEWS.profile,VIEWS.newSession,VIEWS.commSettings,VIEWS.liveSession,VIEWS.transferComm].includes(view);
...
export default function HomeGameApp() {
  ...
  useEffect(()=>{
    if(!league||![VIEWS.league,VIEWS.liveSession].includes(view)||!liveSession)return;
  ...
  const handleJoinCreate=async({tab,code,leagueName,description,buyIn,season,seasonLength,isPublic,locationName,maxPlayers}:any)=>{
    ...
    try{
      if(tab==="join"){
        ...
        const{data:currentPlayers}=await db.from("players").select("id").eq("league_id",lg.id);
        if((currentPlayers||[]).length>=(lg.max_players||12)){showError(`This league is full (${lg.max_players||12} max).`);return;}
  ...
  const handleLeaveLeague=async()=>{
    if(!db||!league||!profile||!authUser)return;
    const isComm=authUser.id===league.commissioner_id||profile.display_name.toLowerCase()===league.commissioner_name?.toLowerCase();
    if(isComm){
      if(players.length<=1){if(window.confirm("You're the only member. Delete this league?"))await db.from("leagues").delete().eq("id",league.id);}
      else setView(VIEWS.transferComm);
      return;
    }
    if(!window.confirm(`Leave ${league.name}?`))return;
    const myPlayer=players.find((p:any)=>p.name.toLowerCase()===profile.display_name.toLowerCase());
    if(myPlayer)await db.from("players").delete().eq("id",myPlayer.id);
    const stored:string[]=JSON.parse(localStorage.getItem(`hg_leagues_${authUser.id}`)||'[]');localStorage.setItem(`hg_leagues_${authUser.id}`,JSON.stringify(stored.filter((id:string)=>id!==league.id)));
    await loadMyLeagues(profile.display_name,authUser.id);showToast("You left the league.");setLeague(null);setView(VIEWS.myLeagues);
  };
  ...
  const handleTransferAndLeave=async()=>{
    ...
    if(myPlayer)await db.from("players").delete().eq("id",myPlayer.id);
    const stored:string[]=JSON.parse(localStorage.getItem(`hg_leagues_${authUser.id}`)||'[]');localStorage.setItem(`hg_leagues_${authUser.id}`,JSON.stringify(stored.filter((id:string)=>id!==league.id)));
    await loadMyLeagues(profile.display_name,authUser.id);showToast("League transferred. You left.");setLeague(null);setView(VIEWS.myLeagues);
  };

  const handleStartSession=async({selectedIds,sessionBuyIn}:any)=>{
    if(!db||!league||!profile)return;
    ...
    // Post to feed
    await db.from("posts").insert({league_id:league.id,author_name:profile.display_name,content:`🃏 Game is live! Buy-in: $${sessionBuyIn} · ${selectedPlayers.length} players seated. Join the live session in the league tab!`});
    showToast("Session started! Posted to feed.");setView(VIEWS.liveSession);
  };
  ...
  const handleEndSession=async({entries,elapsed,chickenDinner}:any)=>{
    ...
    try{
      ...
      for(const e of entries){
        ...
        // Update global profile stats for opted-in players
  ...
  const inLeagueView=[VIEWS.league,VIEWS.feed,VIEWS.standings,VIEWS.profile,VIEWS.newSession,VIEWS.commSettings,VIEWS.liveSession,VIEWS.handRankings,VIEWS.transferComm].includes(view);
  ...
  return(
    <>
      ...
      <div style={{ background:"#0A0A0A",minHeight:"100vh",paddingBottom:inLeagueView?80:0 }}>
        ...
        {view===VIEWS.league&&league&&profile&&<LeagueView league={league} players={players} sessions={sessions} profile={profile} isCommissioner={isCommissioner} onNav={handleNav} onStartSession={()=>setView(VIEWS.newSession)} onBack={()=>setView(VIEWS.myLeagues)} onCommSettings={()=>setView(VIEWS.commSettings)} liveSession={liveSession} onLeaveLeague={handleLeaveLeague}/>}
        {view===VIEWS.transferComm&&league&&profile&&<TransferCommView league={league} players={players} profile={profile} onBack={()=>setView(VIEWS.league)} onTransferred={handleTransferAndLeave}/>}
        ...
        {view===VIEWS.newSession&&league&&<NewSessionView league={league} players={players} onStart={handleStartSession} onBack={()=>setView(VIEWS.league)}/>}
