...
// FIX: Avatar cache with version key so updates propagate immediately
const avatarCache: Record<string, string|null> = {};
export function bustAvatarCache(name: string, url: string|null) {
  avatarCache[name.toLowerCase()] = url;
}
...
function Avatar({ name, url, size = 40 }: { name: string; url?: string | null; size?: number }) {
  ...
  // FIX: re-fetch if url prop changes (own profile update propagates)
  useEffect(() => {
  ...
  return (
    <div style={{ maxWidth: 500, margin: "0 auto" }}>
      <div style={{ background: "linear-gradient(180deg,rgba(20,40,20,0.95) 0%,rgba(10,10,10,0) 100%)", padding: "18px 16px 0" }}>
        ...
        {/* FIX: season done banner */}
        {seasonDone && <div style={{ background: "rgba(224,85,85,0.1)", border: "1px solid rgba(224,85,85,0.3)", borderRadius: 10, padding: "9px 14px", marginBottom: 10, color: "#E05555", fontFamily: "'Space Mono',monospace", fontSize: 11, textAlign: "center" }}>🏁 Season complete — {sessions.length} sessions played</div>}
...
// FIX: properly recalculates player totals when commissioner edits a past session
function SessionDetailView({ session, league, players, isCommissioner, onBack, onSaved, showToast, showError }: any) {
  ...
  // FIX: recalculate player stats by diffing old vs new profits
  const handleSave = async () => {
    ...
    try {
      ...
      for (const e of entries) {
        ...
        if (diff !== 0 && e.players) {
          const p = e.players;
          const oldWon = oldProfit > 0 ? 1 : 0;
          const newWon = newProfit > 0 ? 1 : 0;
          const winDiff = newWon - oldWon;
          const newTotal = (p.total_profit || 0) + diff;
          // Recalculate best_night: only increase, or if this was previously best, fetch all to recalc
          let newBest = p.best_night || 0;
          if (newProfit > newBest) newBest = newProfit;
          await db.from("players").update({
            total_profit: newTotal,
            wins: Math.max(0, (p.wins || 0) + winDiff),
            best_night: newBest,
          }).eq("id", e.player_id);
        }
        // FIX: update chicken dinner count
        ...
        if (wasChicken !== isChicken && e.players) {
          const delta = isChicken ? 1 : -1;
          await db.from("players").update({ chicken_dinners: Math.max(0, (e.players.chicken_dinners || 0) + delta) }).eq("id", e.player_id);
        }
      }
  ...
  return (
    <div style={{ padding: "20px 16px", maxWidth: 500, margin: "0 auto" }}>
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        {editing && <div style={{ marginTop: 12 }}><label style={{ color: "#888", fontSize: 10, fontFamily: "'Space Mono',monospace", marginBottom: 5, display: "block" }}>OVERRIDE BUY-IN AMOUNT</label><input type="number" value={buyInAmount} onChange={e => setBuyInAmount(e.target.value)} style={inp} /></div>}
      </Card>
      <Card style={{ marginBottom: 12 }}>
        <div style={{ color: "#888", fontSize: 10, fontFamily: "'Space Mono',monospace", letterSpacing: 2, marginBottom: 11 }}>RESULTS {editing && <span style={{ color: "#555" }}>— tap profit to edit</span>}</div>
...
function TransferCommView({ league, players, profile, onBack, onTransferred }: any) {
  ...
  // FIX: match commissioner by auth user ID instead of just display name
  const handleTransfer = async () => {
    if (!db || !selected) return; setLoading(true);
    const { data: np } = await db.from("profiles").select("id").ilike("display_name", selected).limit(1);
    const newCommId = np?.[0]?.id || null;
    if (!newCommId) { setLoading(false); return; } // don't transfer if we can't find a valid profile
...
function LiveSessionView({ session, liveEntries, players, profile, isCommissioner, league, onBack, onSubmitEntry, onEndSession }: any) {
  ...
  const handleEnd = async () => {
    ...
    const entries = liveEntries.map((e: any) => ({
      ...
      buy_in: e.buy_in || 0, rebuys: e.rebuys || 0,
      cash_out: isCommissioner ? (cashOuts[e.player_id] !== undefined ? cashOuts[e.player_id] : (e.cash_out || 0)) : (e.cash_out || 0),
      profit: (isCommissioner ? (cashOuts[e.player_id] !== undefined ? cashOuts[e.player_id] : (e.cash_out || 0)) : (e.cash_out || 0)) - ((e.buy_in || 0) + (e.rebuys || 0))
    }));
    await onEndSession({ entries, elapsed, chickenDinner }); setSaving(false);
  };
...
function CommSettingsView({ league, players, onBack, onLeagueUpdated, onLeagueDeleted, showToast, showError }: any) {
  ...
  // FIX: kick also removes from live_entries if a session is active
  const kick = async (id: string, name: string) => {
    if (!db || !window.confirm(`Kick ${name}?`)) return;
    await db.from("players").delete().eq("id", id);
    // Remove from any active live_entries
    await db.from("live_entries").delete().eq("player_id", id);
    showToast(`${name} removed.`); onLeagueUpdated(league);
  };
...
// ─── FEED ──────────────────────────────────────────────
// FIX: refresh feed when tab becomes active
function FeedView({ profile, myLeagues, isActive }: any) {
  ...
  // FIX: reload when tab becomes visible
  useEffect(() => { if (isActive) { loadPosts(); } }, [isActive]);

  const loadPosts = async () => {
    if (!db || myLeagues.length === 0) { setLoading(false); return; } setLoading(true);
    const name = profile.display_name;
    const [{ data: friendData }, { data: postsData }] = await Promise.all([
      db.from("friends").select("requester_name,recipient_name").or(`requester_name.ilike.${name},recipient_name.ilike.${name}`).eq("status", "accepted"),
      db.from("posts").select("*").in("league_id", myLeagues.map((l: any) => l.id)).order("created_at", { ascending: false })
    ]);
...
function ProfileTabView({ profile, myLeagues, isSelf, externalName, onFriends, onLogout, onSendFriendRequest }: any) {
  ...
  const handleSaveName = async () => {
    if (!db || !newName.trim() || newName.trim() === profile.display_name) return; setSavingName(true);
    const { error } = await db.from("profiles").update({ display_name: newName.trim() }).eq("id", profile.id);
    if (!error) {
      // FIX: bust avatar cache for old name, re-cache with new name
  ...
  const handleAvatar = async (e: any) => {
    const f = e.target.files?.[0]; if (!f || !db) return; setUploadingAvatar(true);
    try {
      const ext = f.name.split('.').pop(); const path = `${profile.id}/avatar.${ext}`;
      ...
      // FIX: immediately bust cache so all avatars update
...
export default function HomeGameApp() {
  ...
  const loadMyLeagues = async (displayName: string, userId: string) => {
    ...
    try {
      ...
      if (allIds.length > 0) {
        const { data: lgs } = await db.from("leagues").select("id,name,description,code,season,season_length,buy_in,is_public,location_name,commissioner_id,commissioner_name,max_players").in("id", allIds);
        setMyLeagues((lgs || []).map((lg: any) => ({ ...lg, _myUserId: userId })));
      } else setMyLeagues([]);
  ...
  // FIX: poll for live session on BOTH leagueDetail AND liveSession views
  useEffect(() => {
    if (!currentLeague || !['leagueDetail', 'liveSession'].includes(lsv)) return;
    const t = setInterval(async () => {
      if (!db || !currentLeague) return;
      // Check for live session if on leagueDetail (so banner appears automatically)
      if (lsv === 'leagueDetail') {
        const { data: lsData } = await db.from("sessions").select("*").eq("league_id", currentLeague.id).eq("is_live", true).limit(1);
        if (lsData && lsData.length > 0 && lsData[0].id !== liveSession?.id) {
          setLiveSession(lsData[0]); const { data: le } = await db.from("live_entries").select("*").eq("session_id", lsData[0].id); setLiveEntries(le || []);
        } else if ((!lsData || lsData.length === 0) && liveSession) {
          setLiveSession(null); setLiveEntries([]);
        } else if (liveSession) {
          const { data: le } = await db.from("live_entries").select("*").eq("session_id", liveSession.id); setLiveEntries(le || []);
        }
      }
      if (lsv === 'liveSession' && liveSession) {
        const { data } = await db.from("live_entries").select("*").eq("session_id", liveSession.id); setLiveEntries(data || []);
      }
    }, 8000);
  ...
  const joinLeague = async (lg: any) => {
    ...
    setCurrentLeague({ ...lg, _myUserId: authUser.id }); await loadLeagueData(lg.id);
    // FIX: confirmation toast on joining
    showToast(`Joined ${lg.name}! 🎉`);
    setLsv('leagueDetail');
  };
  ...
  // FIX: check both directions before sending friend request to prevent duplicates
